jQuery(function ($) {
    ...
    // tabs
    $('ul.tabs-doc, ul.tabs').on('click', 'li:not(.current)', function () {
        $(this).addClass('current').siblings().removeClass('current')
            .parents('div.section').find('div.box').eq($(this).index()).fadeIn(1000).siblings('div.box').hide();
    })

    $(document).on('mouseenter', '.service-list-item', function (e) {
        var img = $(this).data('img');
        var info = $(this).data('info');
        if (img) {
            $('.service-list-img img').attr('src', img);
        }
        $('.service-info').hide();
        if (info) {
            $('.service-info.info' + info).show();
        }
    });
    ...
    var owl = $('.about-carousel').owlCarousel({
        loop: true,
        nav: true,
        dots: false,
        items: 1,
        ...
        mouseDrag: false,
        navSpeed: 1000,
        dotsContainer: '.about-carousel-dots'
    });

    $('.about-carousel .owl-next').on('click', function (e) {
        $('.about-carousel-dots .page-nav__item').removeClass('page-nav__item--active');
        var i = $('.owl-item.active div').data('i');
        $('.about-carousel-dots .page-nav__item[data-i=' + i + ']').addClass('page-nav__item--active');
    });

    $('.about-carousel .owl-prev').on('click', function (e) {
        $('.about-carousel-dots .page-nav__item').removeClass('page-nav__item--active');
        var i = $('.owl-item.active div').data('i');
        $('.about-carousel-dots .page-nav__item[data-i=' + i + ']').addClass('page-nav__item--active');
    });

    $('.about-carousel-dots').on('click', 'a', function (e) {
        e.preventDefault();
        e.stopPropagation();
        $('.about-carousel-dots .page-nav__item').removeClass('page-nav__item--active');
        $(this).addClass('page-nav__item--active');
                ...
                $.getScript('//www.google.com/jsapi', function(){
                    google.load('maps', '3', { other_params: 'sensor=false', callback: function(){
                        ...
                        var map = new google.maps.Map(document.getElementById('map'), {
                            zoom: 13,
                            center: new google.maps.LatLng( mapXy[0].trim(), mapXy[1].trim())
                        });

                        var contentString1 = '<div id="content1" class="cont-map">'+
                            '<div class="siteNotice">'+
                            '</div>'+
                            '<div class="bodyContent">'+
                            '<p>'+
                            '3-комнатная <b>3100<span class="rubl">q</span></b>'+
                            '</p>'+
                            '</div>'+
                            '</div>';
                        ...
                        var marker1 = new google.maps.Marker({
                            position: new google.maps.LatLng( mapXy[0].trim(), mapXy[1].trim()),
                        ...
                        var contentString2 = '<div id="content2" class="cont-map">'+
                            '<div class="siteNotice">'+
                            '</div>'+
                            '<div class="bodyContent">'+
                            '<p>'+
                            '1-комнатная <b>2100<span class="rubl">q</span></b> '+
                            '</p>'+
                            '</div>'+
                            '</div>';
                        ...
                        var marker2 = new google.maps.Marker({
                            position: new google.maps.LatLng( mapXy2[0].trim(), mapXy2[1].trim()),
                            icon: '/img/MapMarker.png',
                            map: map
                        });
                        ...
                        latlngbounds.extend(new google.maps.LatLng(mapXy[0].trim(), mapXy[1].trim()));
                        latlngbounds.extend(new google.maps.LatLng(mapXy2[0].trim(), mapXy2[1].trim()));
    ...
    $('#fMenu').each(function(){
        var a = document.querySelector('#fMenu'), b = null, P = 0;
        window.addEventListener('scroll', Ascroll, false);
        document.body.addEventListener('scroll', Ascroll, false);
        function Ascroll() {
            if (b == null) {
    ...
    $(document).on('click', '.doctor-item', function(e){
        ...
        console.log($(this).find('.form-group-select-img').html());
        ...
        submitHandler: function(form) {
            ...
            $('#form-order').ajaxSubmit({
                ...
                success: function(data) {
                    $(form).find('button:submit').removeAttr('disabled').html('Записаться');
                    if (data) {
                        if (data.type == "error") {
                            $(form).find('.alert-error').html(data.data).show();
                        }
                        if (data.type == "ok") {
                            $(form).find('.alert-ok').html(data.data).show();
                            $(form).resetForm();
                        }
                    }
                }
    ...
    $("#form-call").validate({
        submitHandler: function(form) {
            ...
            $(form).find('button:submit').attr('disabled','disabled').html('Подождите...');
            $('#form-call').ajaxSubmit({
                dataType: "json",
                success: function(data) {
                    // $(form).find('button:submit').removeAttr('disabled').html('Перезвоните мне');
                    if (data) {
                        if (data.type == "error") {
                            $(form).find('.alert-error').html(data.data).show();
                            $(form).find('button:submit').removeAttr('disabled').html('Перезвоните мне');
                        ...
                        if (data.type == "ok") {
                            // $(form).find('.alert-ok').html(data.data).show();
                            $(form).find('button:submit').removeAttr('disabled').html('Отправлено!');
                            $(form).resetForm();
                        }
                    }
                }
